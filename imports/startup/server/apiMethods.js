...
apiCall = function (apiUrl, callback) {
  ...
  try {
    ...
    if (dataFromCache) {
      ...
      if (response) {
        return OCache.set(apiUrl, response);
      } else {
...
Meteor.methods({
  ...
  geoCode: function (address) {
    ...
    const apiUrl =
      ...
      address +
      "&key=" +
      Meteor.settings.public.keys.googleServer.key;
...
*/
    ...
    if (gidFromCache && !doc.google_id) {
      ...
      let apiUrl =
        "https://maps.googleapis.com/maps/api/place/add/json?key=" +
        Meteor.settings.public.keys.googleServer.key;
      const params = {};
  ...
  setGID: function (id, google_id) {
    check(id, String);
    check(google_id, String);
    Listings.update({ _id: id }, { $set: { google_id: google_id } });
  },
